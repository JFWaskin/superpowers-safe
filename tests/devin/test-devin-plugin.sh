#!/usr/bin/env bash
# Validate the Devin CLI integration. `devin plugins install JFWaskin/superpowers-safe`
...
#
# This is a fork adaptation of upstream's `tests/devin/test-devin-plugin.sh`,
# which itself mirrors `tests/kimi/test-plugin-manifest.sh`. CI-safe: does not
...
# Fork convention: name is "superpowers-safe" (upstream uses "superpowers").
if manifest.get("name") != "superpowers-safe":
    raise AssertionError(f"plugin name: expected 'superpowers-safe', got {manifest.get('name')!r}")

# Fork must point at the JFWaskin repo, not obra upstream.
if "JFWaskin/superpowers-safe" not in (manifest.get("homepage") or ""):
    raise AssertionError(
        f"homepage must point at JFWaskin/superpowers-safe, got {manifest.get('homepage')!r}"
    )
if "JFWaskin/superpowers-safe" not in (manifest.get("repository") or ""):
    raise AssertionError(
        f"repository must point at JFWaskin/superpowers-safe, got {manifest.get('repository')!r}"
    )
